    ...
    def run_in_series(
        ...
        **kwargs
        ...
        (
            ...
            wide_error,
            sharp_error,
        ) = self.lat_pid_controller.run_in_series(
        ...
        return VehicleControl(
            throttle=throttle, steering=steering, brake=brake, gear=gear
        )
    ...
    def run_in_series(
        ...
        **kwargs
        ...
        direction_vector = np.array(
            [
                -np.sin(np.deg2rad(self.agent.vehicle.transform.rotation.yaw)),
                0,
        ...
        error = np.arccos(
            min(max(v_vec_normed @ w_vec_normed.T, -1), 1)
        )  # makes sure arccos input is between -1 and 1, inclusive
        ...
        k_p, k_d, k_i = PIDFastController.find_k_values(
            config=self.config, vehicle=self.agent.vehicle
        )

        lat_control = float(
            np.clip(
                (k_p * error) + (k_d * _de) + (k_i * _ie),
                self.steering_boundary[0],
