...
def compute_score(carla_runner: CarlaRunner) -> Tuple[float, int, int]:
    ...
    time_elapsed: float = (
        carla_runner.end_simulation_time - carla_runner.start_simulation_time
    )
    num_collision: int = carla_runner.agent_collision_counter
    laps_completed = (
        0 if carla_runner.completed_lap_count < 0 else carla_runner.completed_lap_count
    )
...
def main():
    ...
    table.add_row(
        (f"AVG for {num_trials} trials = {np.average(total_score_array)}", "N/A", "N/A")
    )
    print(table)
